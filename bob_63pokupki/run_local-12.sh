#!/bin/bash
# Если запускаем из /dev.conf/ci/ -- надо контролировать !!!
cd ../..
DIR=$(pwd)
# DIR="/var/srv/srv12"
echo "$DIR"
...
echo "========================================"
cp -pr "$DIR""/dev.conf/config/devsrv/dev12config.public.ts" "$DIR""/client.web/src/configs/config.public.ts"
echo "Код возврата копирования конфигурации public = "$?

echo "========================================="
echo " Step 4 копирование конфигураций private"
echo "========================================="
cp -pr "$DIR""/dev.conf/config/devsrv/dev12config.private.ts" "$DIR""/client.web/src/configs/config.private.ts"
...
#npm run build
#pm2 restart clieqntweb12
#pm2 kill
#pm2 start clientweb12  ---так что-то не хочет
#pm2 start /var/srv/srv12/client.web/build/index.js --name clientweb10 # НЕ работает, версия = 1.0.0
#pm2 start /var/srv/srv12/client.web/build/App.js --name clientweb10 # НЕ работает, версия = 1.0.0
pm2 start ./node_modules/nuxt/bin/nuxt.js --name clientweb12 -- start # работает, версия = 2.15.8
echo "Код возврата = "$?
